...
<div class="card">
    <div class="card-header">
        {{ trans('global.edit') }} {{ trans('cruds.quran.title_singular') }}
    </div>

    <div class="card-body">
        <form method="POST" action="{{ route("admin.qurans.update", [$quran->id]) }}" enctype="multipart/form-data">
            @method('PUT')
            @csrf
            <div class="form-group">
                <label for="ayah_number">{{ trans('cruds.quran.fields.ayah_number') }}</label>
                <input class="form-control {{ $errors->has('ayah_number') ? 'is-invalid' : '' }}" type="text" name="ayah_number" id="ayah_number" value="{{ old('ayah_number', $quran->ayah_number) }}">
            ...
            <div class="form-group">
                ...
                <textarea class="form-control ckeditor {{ $errors->has('ayah') ? 'is-invalid' : '' }}" name="ayah" id="ayah">{!! old('ayah', $quran->ayah) !!}</textarea>
                @if($errors->has('ayah'))
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('surah') ? 'is-invalid' : '' }}" name="surah_id" id="surah_id">
                    @foreach($surahs as $id => $entry)
                        <option value="{{ $id }}" {{ (old('surah_id') ? old('surah_id') : $quran->surah->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
                @if($errors->has('surah'))
