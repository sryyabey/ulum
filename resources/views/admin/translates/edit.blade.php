...
<div class="card">
    <div class="card-header">
        {{ trans('global.edit') }} {{ trans('cruds.translate.title_singular') }}
    </div>

    <div class="card-body">
        <form method="POST" action="{{ route("admin.translates.update", [$translate->id]) }}" enctype="multipart/form-data">
            @method('PUT')
            @csrf
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('lang') ? 'is-invalid' : '' }}" name="lang_id" id="lang_id">
                    @foreach($langs as $id => $entry)
                        <option value="{{ $id }}" {{ (old('lang_id') ? old('lang_id') : $translate->lang->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
                @if($errors->has('lang'))
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('ayah') ? 'is-invalid' : '' }}" type="number" name="ayah" id="ayah" value="{{ old('ayah', $translate->ayah) }}" step="1">
                @if($errors->has('ayah'))
            ...
            <div class="form-group">
                ...
                <textarea class="form-control {{ $errors->has('translate') ? 'is-invalid' : '' }}" name="translate" id="translate">{{ old('translate', $translate->translate) }}</textarea>
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('surah') ? 'is-invalid' : '' }}" name="surah_id" id="surah_id">
                    @foreach($surahs as $id => $entry)
                        <option value="{{ $id }}" {{ (old('surah_id') ? old('surah_id') : $translate->surah->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
                @if($errors->has('surah'))
